...
Tilt the BBG up/down or right/down to find the dot. If the light is at the top then the target is lower and if the light is at the bottom LED 
then the target is higher. Perfect height if all LEDs are on. If the LEDs are red then you need to rotate left as target is more to the left.
If LEDs are green then you need to rotate right as target is more to the right. Blue LEDs represent the BBG is perfectly centered on the target.

Press the joystick downwards to shoot. Do this when all the LEDs are on and blue to shoot the dot correctly. A new dot will be selected if you hit 
it correctly, unless it will stay the same and notify you that you missed it.

Press the joystick to the right to exit the game.
